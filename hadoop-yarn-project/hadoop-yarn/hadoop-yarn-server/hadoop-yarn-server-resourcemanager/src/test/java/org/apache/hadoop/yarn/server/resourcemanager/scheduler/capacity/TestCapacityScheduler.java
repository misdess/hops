...
public class TestCapacityScheduler {
  ...
  public void setUp() throws Exception {
    ...
    when(mockContext.getConfigurationProvider())
 ...
 new NodeUpdateSchedulerEvent(node,
  ...
  private void checkQueueCapacities(CapacityScheduler cs, float capacityA,
      float capacityB) {
  ...
  public void testParseQueue() throws IOException {
    ...
    CapacitySchedulerConfiguration conf = new CapacitySchedulerConfiguration();
    setupQueueConfiguration(conf);
    conf.setQueues(CapacitySchedulerConfiguration.ROOT + ".a.a1",
        new String[]{"b1"});
  ...
  public void testCapacitySchedulerInfo() throws Exception {
    ...
    List<QueueUserACLInfo> userACLInfo =
        resourceManager.getResourceScheduler().getQueueUserAclInfo();
    Assert.assertNotNull(userACLInfo);
    for (QueueUserACLInfo queueUserACLInfo : userACLInfo) {
      Assert.assertEquals(
          getQueueCount(userACLInfo, queueUserACLInfo.getQueueName()), 1);
    }

  }

  private int getQueueCount(List<QueueUserACLInfo> queueInformation,
      String queueName) {
    int result = 0;
 ...
 new AppAttemptAddedSchedulerEvent(
  ...
  public void testGetAppsInQueue() throws Exception {
    ...
    Application application_1 =
        new Application("user_0", "a2", resourceManager);
    application_1.submit();

    Application application_2 =
        new Application("user_0", "b2", resourceManager);
  ...
  public void testAllocateDoesNotBlockOnSchedulerLock() throws Exception {
    ...
    Assert.assertEquals(attempt.getState(),
  ...
  public void testNumClusterNodes() throws Exception {
    ...
    cs.setConf(conf);
    RMContextImpl rmContext =  new RMContextImpl(null, null, null, null, null,
        null, null, null, new ClientToAMTokenSecretManagerInRM(), null, null);
    
    rmContext.setContainerTokenSecretManager(new 
        RMContainerTokenSecretManager(conf, rmContext));
    rmContext.setNMTokenSecretManager(new 
        NMTokenSecretManagerInRM(conf, rmContext));
    ...
    TransactionState transactionState=
            new TransactionStateImpl( TransactionState.TransactionType.RM);
    cs.handle(new NodeAddedSchedulerEvent(n1, transactionState));
    cs.handle(new NodeAddedSchedulerEvent(n2, transactionState));
    assertEquals(2, cs.getNumClusterNodes());

    cs.handle(new NodeRemovedSchedulerEvent(n1, transactionState));
    assertEquals(1, cs.getNumClusterNodes());
    cs.handle(new NodeAddedSchedulerEvent(n1, transactionState));
    assertEquals(2, cs.getNumClusterNodes());
    cs.handle(new NodeRemovedSchedulerEvent(n2, transactionState));
    cs.handle(new NodeRemovedSchedulerEvent(n1, transactionState));
    assertEquals(0, cs.getNumClusterNodes());
